      ...
      try {
        ...
        if (mode === 'archive') {
          const archiveRes = await fetch('/api/archive', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              celebrity: analysis.celebrity_match.celebrity,
              year: analysis.celebrity_match.year,
            }),
          });
          if (archiveRes.ok) {
        ...
        // Make sure we let the dramatic loader play out a bit
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-6 md:px-12 relative overflow-hidden">
        {/* Scan line */}
        <div className="absolute inset-x-0 h-px bg-oxblood/40 scanline" />
        ...
        <div className="max-w-3xl w-full">
          ...
          {/* Confidence */}
          <div className="mb-10">
            <div className="flex items-baseline justify-between mb-2 font-mono text-[10px] uppercase tracking-widest text-ink/60">
              <span>Cultural confidence</span>
              <span className="text-oxblood text-2xl font-display">
                {String(confidence).padStart(2, '0')}%
              </span>
            </div>
            <div className="h-px bg-ink/20 relative">
              <div
                className="absolute inset-y-0 left-0 bg-oxblood transition-all duration-300 ease-out"
          ...
          <ol className="space-y-2 font-mono text-sm">
            {STAGES.map((stage, idx) => {
              ...
              return (
                ...
                >
                  <span className="font-mono text-[10px] tracking-widest text-ink/50 w-8">
          ...
          {errorMsg && (
            <div className="mt-10 border border-oxblood p-4 font-mono text-xs text-oxblood">
